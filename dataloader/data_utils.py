...
def set_up_datasets(args):
    ...
    if args.dataset == 'mini_imagenet':
        ...
        args.shot = 5
        args.sessions = 9
    args.Dataset=Dataset #arg.Dataset 是dataloader下cifar100中cifar.py
    return args
...
def get_transform(args):
    if args.dataset == 'cifar100':
        ...
        normalize = transforms.Normalize(mean=[0.4815, 0.4578, 0.4082],
    ...
    #多尺度裁剪：size_crop=[32,18], min_scale_crops=[0.9,0.2],max_scale_crops=[1.0,0.7],num_crops=[2,4], args.constrained_cropping=true
    #大crop尺寸是32*32，从原图中裁剪29*29区域再resize为32*32，小的crop尺寸是18*18，从原图中裁剪6*6再resize为18*18，而且小的crop是在大crop中取得
    if len(args.auto_augment) == 0:
        ...
        secondary_transform = transforms.Compose([
            ...
            transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8), #80%概率去采用颜色扰动
            transforms.RandomGrayscale(p=0.2), #20%概率变化为灰度图
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'cifar100':
        ...
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True, index=class_index, base_sess=True,
                                         crop_transform=crop_transform, secondary_transform=secondary_transform, use_text=args.use_text)
...
def get_new_dataloader(args,session):
    ...
    if args.dataset == 'cifar100':
        ...
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=False, index=class_index, base_sess=False,         crop_transform=crop_transform, secondary_transform=secondary_transform, use_text=args.use_text)
    if args.dataset == 'cub200':
    ...
    class_new = get_session_classes(args, session) #class_list=np.arange(args.base_class + session * args.way)
